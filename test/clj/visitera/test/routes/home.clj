(ns visitera.test.routes.home
  (:require
   ...
   [visitera.routes.home :refer [login-handler logout-handler
                                 register-handler!]]))
...
(deftest register-handler-test
  (testing "Bad input:"
    ...
    (testing "should return an error if user already exists"
      (let [req {:params {:email "some@email.com" :password "somepass"}}
            response (register-handler! req)]
...
(deftest login-handler-test
  ...
  (testing "should add identity field to session and redirect to '/' for the correct input"
